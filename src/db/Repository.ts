...
export class Repository<TEntity extends IGuildRelated> {
		...
		return collection.insertOne(entity)
			.then((result) => {
				if (result.result.ok !== 1) {
					this.logger.logError(this.constructor.name, this.insert.name, Constants.emptyString)
					return false
				}
				return true
			})
	...
	protected async update(filter: FilterQuery<TSchema>, update: UpdateQuery<TSchema>): Promise<boolean> {
		...
		return collection.updateOne(filter, update)
			.then((result) => this.handleOperationResult(result))
	}
	...
	private handleOperationResult(operationResult: DeleteWriteOpResultObject | UpdateWriteOpResult | InsertOneWriteOpResult<any>): boolean {
